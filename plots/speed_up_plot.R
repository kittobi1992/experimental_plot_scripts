...
detailed_speed_up_vs_single_threaded_plot <- function(dataframes, 
  ...
  speed_up <- speed_up + geom_line(aes(y = rolling_gmean_speed_up), size = 2 * plot_line_size(latex_export)) +
    ...
    guides(colour = guide_legend(title=NULL, ncol = length(levels(factor(speed_up_df$num_threads))), byrow = 
                                   F, keyheight = .5, title.hjust=0))
  
  if ( hide_axis_title  ) {
    speed_up <- speed_up +  theme(axis.title.y = element_blank(),

...
running_time_per_pin_box_plot <- function(dataframes, 
  ...
  if ( contains_invalid_results ) {
    ...
    y_breaks <- add_infeasible_break(y_breaks, infeasible_value, show_infeasible_tick & contains_infeasible_results, latex_export)
    y_labels <- add_infeasible_label(y_labels, infeasible_value, show_infeasible_tick & contains_infeasible_results, latex_export)
    y_breaks <- add_timeout_break(y_breaks, timeout_value, show_timeout_tick & contains_timeout_results, latex_export)
    y_labels <- add_timeout_label(y_labels, timeout_value, show_timeout_tick & contains_timeout_results, latex_export)
  ...
  running_time = ggplot(result, aes(x=algorithm, y=avg_time_per_pin)) +
    ...
    labs(x="", y=paste("Time per pin",ifelse(latex_export, to_latex_math_mode("[\\mu s]",latex_export), "[µs]"),sep=" ")) +
